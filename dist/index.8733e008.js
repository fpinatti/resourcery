/* global chrome */ document.addEventListener('DOMContentLoaded', ()=>{
    let fullData;
    let userEmail;
    ...
    const buildPostsList = (postsList)=>{
        ...
        postsList.forEach((item, idx)=>{
            if (!item['content:encoded']) item['content:encoded'] = item.description;
    ...
    const fetchResources = (userRole)=>{
        ...
        fetch(rssFetch, options).then((response)=>response.json()
        ).then((data)=>{
            buildPostsList(data);
        }).catch(console.error);
    };
    const getLocation = ()=>{
        ...
        return fetch(location, options).then((response)=>response.json()
        ).then((data)=>{
    ...
    const fetchWeather = ()=>{
        ...
        fetch(endpointWeather, options).then((response)=>response.json()
        ).then((data)=>{
    ...
    const setGreetings = ()=>{
        ...
        if (hourOfDay > 5 && hourOfDay <= 12) greetString = 'Good morning';
        if (hourOfDay > 12 && hourOfDay <= 18) greetString = 'Good afternoon';
        if (hourOfDay > 18 && hourOfDay <= 23) greetString = 'Good night';
        if (hourOfDay >= 0 && hourOfDay < 5) greetString = 'It\'s late';
    ...
    const addListeners = ()=>{
        ...
        chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
            if (request.action === 'chrome-message') onGetChromeMessage(request.message, request.data);
        });
    };
    ...
    const buildCalendarEvents = (objData)=>{
        ...
        sortEvents.forEach((event)=>{
            ...
            if (event.eventLive) clonedEvent.querySelector('.calendar-event-item').classList.add('event-live');
    ...
    const getPrettyHourMinute = (strDate)=>{
        ...
        if (tmpDate.getMinutes()) prettyTime = `${prettyTime}${tmpDate.getMinutes()}m`;
        return prettyTime;
    };
    ...
    const getAttendeeResponseStatus = (attendees)=>{
        ...
        attendees.forEach((attendee)=>{
            if (attendee.email === userEmail) attendeeStatus = attendee.responseStatus;
        });
    ...
    const addMillisecondsPropertyToEvents = (events)=>{
        events.forEach((event)=>{
            ...
            // "12:00:00"
            const ms = Number(hour.split(':')[0]) * 3600000 + Number(hour.split(':')[1]) * 60000;
    ...
    const upperCaseNameFirstLetters = (name)=>{
        try {
            ...
            for(let i = 0; i < splitStr.length; i++)if (splitStr[i].length > 1) splitStr[i] = splitStr[i].charAt(0).toUpperCase() + splitStr[i].substring(1);
